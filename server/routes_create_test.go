...
func TestShardedGGUF(t *testing.T) {
	...
	t.Run("single full gguf", func(t *testing.T) {
		...
		mf, err := manifest.ParseNamedManifest(model.ParseName("test-single-full"))
		if err != nil {
			t.Fatalf("parse manifest: %v", err)
		}
	...
	t.Run("complete split gguf", func(t *testing.T) {
		...
		mf, err := manifest.ParseNamedManifest(model.ParseName("test-complete-split"))
		if err != nil {
			t.Fatalf("parse manifest: %v", err)
		}
		for i, layer := range mf.Layers {
	...
	t.Run("complete split misordered gguf", func(t *testing.T) {
		...
		mf, err := manifest.ParseNamedManifest(model.ParseName("test-complete-split-misorder"))
		if err != nil {
			t.Fatalf("parse manifest: %v", err)
		}
		for i, layer := range mf.Layers {
	...
	t.Run("mixed full and split gguf", func(t *testing.T) {
		...
		mf, err := manifest.ParseNamedManifest(model.ParseName("test-full-split-mixing"))
		if err != nil {
			t.Fatalf("parse manifest: %v", err)
		}
		for i, layer := range mf.Layers {
