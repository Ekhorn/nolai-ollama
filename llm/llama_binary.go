...
func llamaCppBinaryCandidates(name string, search llamaCppBinarySearch) []string {
	...
	// Also search GPU runner subdirectories (e.g. rocm_v7_2/) so that
	// binaries built against a GPU backend are found before the CPU fallback.
	if search.libOllamaPath != "" {
		if entries, err := os.ReadDir(search.libOllamaPath); err == nil {
			for _, e := range entries {
				if e.IsDir() {
					add(filepath.Join(search.libOllamaPath, e.Name()))
				}
			}
		}
	}
